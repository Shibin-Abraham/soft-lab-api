<?php include("../../config/constants.php");
    ...
    if(!empty($allheaders['Authorization'])){
        ...
        if($auth_result['statuscode'] === 200 && $auth_result['status'] === '1'){
            if(isset($data->itemno,$data->bId,$data->date,$data->time)){
                $sql = "UPDATE `borrowers` SET `return_status` = '1' WHERE `id` = $data->bId;";
                $result = mysqli_query($conn,$sql);
                if($result){
                    $name = $auth_result['name'];
                    $email = $auth_result['email'];
                    $sql = "INSERT INTO `recent_activities` (`id`, `name`, `email`, `date`, `time`, `details`, `operation`) VALUES (NULL, '$name', '$email', '$data->date', '$data->time', '$data->itemno', 'returned');";
                    $result = mysqli_query($conn,$sql);
                    if($result){
                        $response = array(
